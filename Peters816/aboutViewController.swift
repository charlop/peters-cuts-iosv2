...
class aboutViewController: UIViewController {
    ...
    func getNewHours() {
        postController.getNewHours({ (newHours:(errorNum: CONSTS.ErrorNum, messageText: String)) -> Void in
            let msgLength = newHours.messageText.count
            
            if(newHours.errorNum == CONSTS.ErrorNum.NO_ERROR && msgLength >= 3) {
                self.userDefaults.saveHoursText(hoursText: newHours.messageText)
    ...
    func getNewAddr() {
        postController.getNewAddr({ (newAddr:(errorNum: CONSTS.ErrorNum, messageText: String)) -> Void in
            let msgLength = newAddr.messageText.count
            
            if(newAddr.errorNum == CONSTS.ErrorNum.NO_ERROR && msgLength >= 3) {
                self.userDefaults.saveAddrUrl(addrUrl: newAddr.messageText)
